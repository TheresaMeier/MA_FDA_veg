...
###
# Ideen/TODOs:
# - XX mit smooth.pos statt smooth.basis --> geht nicht, da 'System für den Rechner singulär wird' für nderiv = 4 (nderiv = 3 bedeutet fast linearer fit)
# - XX für anderes norder und/order nderiv --> für norder = 6 (sollte so bleiben) und nderiv = 3: hauptsächliche Änderung: Anstieg in Anfangsjahren ist raus. PCs sehr ähnlich nach Anfangsjahren, PC scores quasi gleich --> kein großer Unterschied für die Interpretation
# - XX für anderes Intervall
# - evtl: für penalized PCs (?) (Frage an Thomas)
# - XX für alle Patches zusammen
# - Plots PC1 vs. PC2 für Lon (evtl. auch Lat)
# - XX Registration
# - XX verschiedene Lambdas ausprobieren: most appropriate fit for chosen lambda (by visual inspection)
# - MFPCA (?)m
# - auf totale cmass Werte gehen? (Frage an Thomas)
# - store fd objects as RDS files
