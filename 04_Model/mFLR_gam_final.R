...
colnames(combined_d)[c(1:2,28:37)] = c("PC1", "PC2", "PC1_temp", "PC1_temp_min", "PC1_temp_max",
...
gam_final = gam(list(PC1 ~ s(Lon,Lat) +
                   ...
                   PC1_temp + #PC1_temp_min + PC1_temp_max +
                   PC1_precip #+ PC1_nuptake_total +
                   # PC1_nuptake_BNE + PC1_nuptake_IBS + PC1_nuptake_otherC + PC1_nuptake_TeBS + PC1_nuptake_Tundra
                 , 
                 PC2 ~ s(Lon,Lat) +
                   sand_fraction + silt_fraction + bulkdensity_soil + ph_soil + soilcarbon +
                   Scenario + time_since_dist +
...
pdf(paste0("Scripts/Plots/Model/Evaluation/pdf/True_vs_fitted_train", run, ".pdf"), width = 10, height = 5)
par(mfrow=c(1,2))
...
############################## Smooth effects ##################################
pdf(paste0("Scripts/Plots/Model/Evaluation/pdf/gam_final_smooth_effect.pdf"), width = 10, height = 5)
par(mfrow = c(1,2))
plot(gam_final)
mtext("Smooth effects for PC 1 (left) and PC 2 (right)", side = 3, line = -1.5, outer = TRUE, font = 2, cex = 1.1)
dev.off()

png(paste0("Scripts/Plots/Model/Evaluation/png/gam_final_smooth_effect.png"), width = 1000, height = 500)
par(mfrow = c(1,2))
plot(gam_final)
mtext("Smooth effects for PC 1 (left) and PC 2 (right)", side = 3, line = -1.5, outer = TRUE, font = 2, cex = 1.1)
dev.off()
...
# Plot the fit
pdf(paste0("Scripts/Plots/Model/Evaluation/pdf/Parameters", run, ".pdf"), width = 10, height = 5)
leg.text = c(
  expression(A == 11.8 ),
  expression(k == 0.8 ),
  expression(x[0] == 0 ),
...
pdf(paste0("Scripts/Plots/Model/Evaluation/pdf/Parameters", run, "_orig.pdf"), width = 5, height = 5)
par(mfrow = c(1, 1))
...
pdf(paste0("Scripts/Plots/Model/Evaluation/pdf/Parameters", run, "_params.pdf"), width = 5, height = 5)
# Construct the legend text using expression
leg.text = c(
  expression(A == 12.26),
  expression(k == 0.66),
...
pdf(paste0("Scripts/Plots/Model/Evaluation/pdf/residuals", run, ".pdf"), width = 10, height = 10)
par(mfrow = c(2, 2)) 
plot(generalized_logistic(fitted(gam_trafo)[,1], A = params["A"], k = params["k"], x0 = params["x0"], C = params["C"]),
...
pdf(paste0("Scripts/Plots/Model/Evaluation/pdf/gam_trafo_smooth_effect.pdf"), width = 10, height = 5)
par(mfrow = c(1,2))
plot(gam_trafo)
mtext("Smooth effects for PC 1 (left) and PC 2 (right)", side = 3, line = -1.5, outer = TRUE, font = 2, cex = 1.1)
dev.off()

png(paste0("Scripts/Plots/Model/Evaluation/png/gam_trafo_smooth_effect.png"), width = 1000, height = 500)
par(mfrow = c(1,2))
plot(gam_trafo)
mtext("Smooth effects for PC 1 (left) and PC 2 (right)", side = 3, line = -1.5, outer = TRUE, font = 2, cex = 1.1)
dev.off()
...
ggsave(paste0("Scripts/Plots/Model/Evaluation/pdf/Correlations.pdf"), width = 15, height = 12)
...
tidy_model <- tidy(gam_trafo, conf.int = TRUE, parametric = TRUE)
tidy_model$term <- factor(tidy_model$term, levels = tidy_model$term)

## PC 1
ggplot(tidy_model[c(1:27),], aes(x = estimate, y = term)) +
  geom_point() +
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = 0.2) +
  labs(title = "Coefficient Plot - PC 1",
       x = "Estimates",
       y = "Predictors") +
...
ggsave(paste0("Scripts/Plots/Model/Evaluation/pdf/Effectplot_PC1.pdf"), width = 8, height = 5)
ggsave(paste0("Scripts/Plots/Model/Evaluation/png/Effectplot_PC1.png"), width = 10, height = 8)

tidy_model_small = tidy_model[tidy_model$conf.low > -0.1 & tidy_model$conf.high < 0.1,]

# Zoom in on very small ones
ggplot(tidy_model_small[c(1:15),], aes(x = estimate, y = term)) +
  geom_point() +
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = 0.2) +
  labs(title = "Coefficient Plot - PC 1 - Zoom on small effects",
       x = "Estimates",
       y = "Predictors") +
  geom_vline(xintercept = 0, color = "grey20") +
  theme_bw() +
...
ggsave(paste0("Scripts/Plots/Model/Evaluation/pdf/Effectplot_PC1_zoom.pdf"), width = 8, height = 5)
ggsave(paste0("Scripts/Plots/Model/Evaluation/png/Effectplot_PC1_zoom.png"), width = 10, height = 8)

## PC 2
ggplot(tidy_model[c(28:54),], aes(x = estimate, y = term)) +
  geom_point() +
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = 0.2) +
  labs(title = "Coefficient Plot - PC 2",
       x = "Estimates",
       y = "Predictors") +
  geom_vline(xintercept = 0, color = "grey20") +
  theme_bw() +
...
ggsave(paste0("Scripts/Plots/Model/Evaluation/pdf/Effectplot_PC2.pdf"), width = 8, height = 5)
ggsave(paste0("Scripts/Plots/Model/Evaluation/png/Effectplot_PC2.png"), width = 10, height = 8)

# Zoom in on very small ones
ggplot(tidy_model_small[c(16:30),], aes(x = estimate, y = term)) +
  geom_point() +
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = 0.2) +
  labs(title = "Coefficient Plot - PC 2 - Zoom on small effects",
       x = "Estimates",
       y = "Predictors") +
  geom_vline(xintercept = 0, color = "grey20") +
  theme_bw() +
...
ggsave(paste0("Scripts/Plots/Model/Evaluation/pdf/Effectplot_PC2_zoom.pdf"), width = 8, height = 5)
ggsave(paste0("Scripts/Plots/Model/Evaluation/png/Effectplot_PC2_zoom.png"), width = 10, height = 8)


############# Get standardized confidence intervals

tidy_model_standardized = tidy_model %>%
  mutate(ci_length = conf.high - conf.low,
         std_ci_length =  std.error/ci_length,
         std_conf.low = estimate - (std_ci_length * 2),
         std_conf.high = estimate + (std_ci_length * 2))

## PC 1
ggplot(tidy_model_standardized[c(1:27),], aes(x = estimate, y = term)) +
  geom_point() +
  geom_errorbarh(aes(xmin = std_conf.low, xmax = std_conf.high), height = 0.2) +
  labs(title = "Coefficient Plot - PC 1",
       x = "Estimates",
       y = "Predictors") +
  geom_vline(xintercept = 0, color = "grey20") +
...
t_values_PC1 = data.frame(var = attr(summary_gam_trafo$p.t, "names")[1:17], p.t = summary_gam_trafo$p.t[1:17])
t_values_PC1$var <- factor(t_values_PC1$var, levels = t_values_PC1$var)

## PC 1
ggplot(t_values_PC1[c(2:17),], aes(x = p.t, y = var)) +
...
t_values_PC2 = data.frame(var = attr(summary_gam_trafo$p.t, "names")[1:17], p.t = summary_gam_trafo$p.t[18:34])
t_values_PC2$var <- factor(t_values_PC2$var, levels = t_values_PC2$var)

ggplot(t_values_PC2[c(2:17),], aes(x = p.t, y = var)) +
  geom_bar(stat = "identity", aes(fill = p.t > 0)) +
  scale_fill_manual(values = c("TRUE" = "darkred", "FALSE" = "cornflowerblue")) +
  theme_bw() + theme(legend.position = "none",
                     plot.title = element_text(size = 15, face = "bold", hjust = 0.5)) +
...
df_smooth = data.frame(Lon = d_train_trafo$Lon, Lat = d_train_trafo$Lat, values = predict(gam_trafo, type = "terms")[,29])

ggplot(df_smooth, aes(x = Lon, y = Lat, color = values)) +
  geom_point(size = 3, shape = 21, fill = "white", color = "black") +  # Larger size with black outline
  geom_point(size = 2.5, aes(fill = values > 0)) +  # Original size with fill color based on values
  scale_color_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0, 
                        limits = c(min(df_smooth$values), max(df_smooth$values)),
                        breaks = c(-1, 0, 1),
...
ggmap(worldmap) +
  geom_point(data = df_smooth, aes(x = Lon, y = Lat, color = values), size = 1) +
  ...
  ylab("Latitude") +
  scale_color_gradient2(name = "Values", low = "blue", mid = "white", high = "red", midpoint = 0, 
                        limits = c(min(df_smooth$values), max(df_smooth$values)),
                        breaks = c(-1, 0, 1),
                        labels = c("-", "", "+")) +
  theme_bw() +
...
ggsave(paste0("Scripts/Plots/Model/Evaluation/pdf/map_smooth.pdf"), width = 10, height = 8)
