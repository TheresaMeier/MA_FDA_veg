smooth.pos_TM <- function(argvals, y, WfdParobj, wtvec=rep(1,n), conv=1e-4,
                       iterlim=50, dbglev=1) {
  ...
  #  The function W(x) is expanded by the basis in functioanal data object
  #    Wfdobj.
  ...
  for (ivar in 1:nvar) {
    ...
    for (icurve in 1:ncurve) {
      if (ndim == 2) {
        ...
        yi    <- y[,icurve,ivar]
        cveci <- coef[,icurve,ivar]
      }
      
      #  evaluate log likelihood
      #    and its derivatives with respect to these coefficients
      
      result <- PENSSEfun_TM(argvals, yi, basisobj, cveci, Kmat, wtvec)
      PENSSE   <- result[[1]]
      DPENSSE  <- result[[2]]
      ...
      hmat0 <- PENSSEhess_TM(argvals, yi, basisobj, cveci, Kmat, wtvec)
      ...
      if (iterlim == 0) {
        ...
        for (iter in 1:iterlim) {
          ...
          for (stepiter in 1:MAXSTEPITER) {
            ...
            result   <- PENSSEfun_TM(argvals, yi, basisobj, cvecnew, Kmat, wtvec)
            PENSSE   <- result[[1]]
            DPENSSE  <- result[[2]]
            Flisti$f <- PENSSE
            gvecnew  <- DPENSSE
          ...
          #  compute the Hessian
          hmat <- PENSSEhess_TM(argvals, yi, basisobj, cveci, Kmat, wtvec)
...
PENSSEfun_TM <- function(argvals, yi, basisobj, cveci, Kmat, wtvec) {
...
PENSSEhess_TM <- function(argvals, yi, basisobj, cveci, Kmat, wtvec) {
