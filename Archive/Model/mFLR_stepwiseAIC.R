...
multivariate_AIC <- function(model) {
  ...
  residuals_matrix <- as.matrix(model$residuals)
...
stepwise_selection <- function(initial_model, direction = "both", trace = FALSE) {
  current_model <- initial_model
  current_AIC <- multivariate_AIC(current_model)
  ...
  repeat {
    ...
    if (direction %in% c("forward", "both")) {
      available_predictors <- setdiff(names(d_train), c("PC1", "PC2", "clay_fraction", attr(current_model$terms,"term.labels")))
      for (predictor in available_predictors) {
        formula <- as.formula(paste("cbind(PC1, PC2) ~", paste(c(names(coef(current_model))[-1], predictor), collapse = " + ")))
        model <- lm(formula, data = d_train)
        candidates <- c(candidates, list(model))
        candidate_AIC <- c(candidate_AIC, multivariate_AIC(model))
      }
    }
    
    # Remove predictors
    if (direction %in% c("backward", "both")) {
      current_predictors <- attr(current_model$terms,"term.labels")
      if (length(current_predictors) > 1) {
        for (predictor in current_predictors) {
          formula <- as.formula(paste("cbind(PC1, PC2) ~", paste(setdiff(current_predictors, predictor), collapse = " + ")))
          model <- lm(formula, data = d_train)
          candidates <- c(candidates, list(model))
          candidate_AIC <- c(candidate_AIC, multivariate_AIC(model))
        }
    ...
    if (length(candidate_AIC) == 0) break  # No more candidates to evaluate
    
    best_candidate_idx <- which.min(candidate_AIC)
    if (candidate_AIC[best_candidate_idx] < best_AIC) {
      best_AIC <- candidate_AIC[best_candidate_idx]
      best_model <- candidates[[best_candidate_idx]]
      if (trace) cat("Step AIC:", best_AIC, "\n")
    } else {
...
final_model <- stepwise_selection(lm_1, direction = "both", trace = TRUE)

setdiff(attr(lm_1$terms,"term.labels"),attr(final_model$terms,"term.labels"))
# Print summary of the final model
summary(final_model)
saveRDS(final_model, paste0("Scripts/MA_FDA_veg/04_Model/ModelRDS/lm_stepwise.rds"))


############################# Model evaluation #################################

residuals_pc1 <- residuals(final_model)[, 1]
residuals_pc2 <- residuals(final_model)[, 2]

# Create residual plots for PC1
pdf(paste0("Scripts/Plots/Model/Evaluation/pdf/residuals", run, ".pdf"), width = 8, height = 8)
par(mfrow = c(2, 2)) 
plot(fitted(final_model)[, 1], residuals_pc1, main = "Residuals vs. Fitted PC 1 scores",
...
plot(fitted(final_model)[, 2], residuals_pc2, main = "Residuals vs. Fitted PC 2 scores",
     xlab = "Fitted values", ylab = "Residuals", pch = 16)
abline(h = 0, col = "red")
...
predictions = predict(final_model, newdata = d_test %>% dplyr::select(-clay_fraction))
